  ...
  describe("Joining and Leaving Groups", () => {
    ...
    describe("#withdrawBalance", () => {
      it("should deposit right amount in user's account", () => {
        ...
        return web3.eth.getBalance(account_three).then(balance => {
          ...
          assert.equal(parseInt(oldAcctBalance) - gasUsed + 100, balance.toNumber());
          return groupBuy.getWithdrawableBalance({from: account_three});
        }).then(balance => {
    ...
    describe("#withdrawFromTokenGroup", () => {
      it("should remove contribution record", () => {
        ...
        return groupBuy.contributeToTokenGroup(tokenId, {
          ...
          expect(arr).to.not.include(tokenId);
          return groupBuy.getContributionBalanceForTokenGroup(tokenId, {from: account_three});
        }).then(balance => {
      ...
      it("should block withdrawal if not part of group", () => {
        let tokenId = 0;

        return expectThrow(groupBuy.leaveTokenGroup(tokenId, {
          from: account_three
        }));
      });

      it("should block withdrawal if token purchased", () => {
        let tokenId = 1;
